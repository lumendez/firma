class ConstanciaDocumentosController < ApplicationController
  ...
  def index
    base_scope = constancia_scope_for_user
  ...
  private
    ...
    def constancia_scope_for_user
      role = current_user.role
    
      if role.nombre == "Dirección"
        ConstanciaDocumento
          .where(firma_direccion: nil, constancia_emitida: true)
          .order("numero_oficio DESC")
    
      elsif role.nombre == "Unidad Academica"
        nombre_completo = ConstanciaDocumento::UNIDADES_ACADEMICAS[role.descripcion]
    
        if nombre_completo
          ConstanciaDocumento
            .where(unidad_academica: nombre_completo)
            .order("numero_oficio DESC")
        else
          # Rol desconocido: fallback seguro o error explícito
          ConstanciaDocumento.none
        end
